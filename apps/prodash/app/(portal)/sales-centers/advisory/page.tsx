...
export default function AdvisorySalesCenterPage() {
  const router = useRouter()
  const { data: allClients, loading } = useCollection<ClientRow>(clientsQuery, 'advisory-clients')
  const [search, setSearch] = useState('')

  const advisoryClients = useMemo(() => {
    let result = allClients.filter(c =>
      (c.account_types || []).some(t => {
        const norm = (t || '').toLowerCase()
        return norm === 'bd' || norm === 'ria' || norm === 'bd/ria'
      })
    )
    if (search) {
    ...
    <div className="mx-auto max-w-6xl">
      ...
      {advisoryClients.length > 0 ? (
        <div className="mt-4 overflow-x-auto rounded-lg border border-[var(--border)]">
          <table className="w-full text-sm">
            ...
            <tbody>
              {advisoryClients.slice(0, 50).map(c => (
                <tr
      ...
      <div className="mt-6 rounded-xl border border-dashed border-[var(--border)] bg-[var(--bg-secondary)] p-6">
        <div className="flex items-center gap-3">
          <span className="material-icons-outlined text-2xl text-[var(--text-muted)]">pie_chart</span>
          <div>
            <p className="text-sm font-medium text-[var(--text-primary)]">Advisory Integration</p>
            <p className="text-xs text-[var(--text-muted)]">
              Portfolio summaries, performance tracking, and rebalancing tools — integration with Schwab/RBC custodians planned via Gradient.
            </p>
          </div>
        </div>
      </div>
