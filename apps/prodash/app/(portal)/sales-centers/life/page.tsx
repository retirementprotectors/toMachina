...
export default function LifeSalesCenterPage() {
  const router = useRouter()
  const { data: allClients, loading } = useCollection<ClientRow>(clientsQuery, 'life-clients')
  const [search, setSearch] = useState('')

  // Filter to clients with life accounts
  const lifeClients = useMemo(() => {
    let result = allClients.filter(c =>
      (c.account_types || []).some(t => (t || '').toLowerCase() === 'life')
    )
    if (search) {
  ...
  if (loading) {
    return (
      <div className="mx-auto max-w-6xl">
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">Life Insurance</h1>
    ...
    <div className="mx-auto max-w-6xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--text-muted)]">
            {lifeClients.length} client{lifeClients.length !== 1 ? 's' : ''} with life accounts
          </p>
        </div>
      </div>
      ...
      <div className="mt-4">
        <input
          ...
          placeholder="Search life clients..."
      ...
      {lifeClients.length > 0 ? (
        ...
        <div className="mt-6 flex flex-col items-center justify-center rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-card)] py-16">
          <span className="material-icons-outlined text-4xl text-[var(--text-muted)]">shield</span>
          <p className="mt-3 text-sm text-[var(--text-muted)]">
            {search ? 'No life clients match your search.' : 'No clients with life insurance accounts found.'}
          </p>
        </div>
      )}
      ...
      <div className="mt-6 rounded-xl border border-dashed border-[var(--border)] bg-[var(--bg-secondary)] p-6">
        <div className="flex items-center gap-3">
          <span className="material-icons-outlined text-2xl text-[var(--text-muted)]">rate_review</span>
          <div>
            <p className="text-sm font-medium text-[var(--text-primary)]">Life Insurance Quoting</p>
            <p className="text-xs text-[var(--text-muted)]">
              Premium comparison and illustration tools — integration planned. Contact your manager for current quoting workflows.
            </p>
          </div>
        </div>
      </div>
