...
/* DEX reads from communications collection to show document-related comms,
   and from case_tasks for document-related tasks. When document collections
   are migrated, this will switch to reading from those. */
...
export default function DexPage() {
  const { data: comms, loading: commsLoading } = useCollection<CommRecord>(commsQuery, 'dex-comms')
  const { data: tasks, loading: tasksLoading } = useCollection<TaskRecord>(tasksQuery, 'dex-tasks')
...
function PipelineStage({ icon, label, description, status }: {
  icon: string; label: string; description: string; status: 'active' | 'planned'
}) {
  return (
    <div className="flex flex-col items-center rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-4 text-center">
      ...
      <span
        ...
        style={{
          background: status === 'active' ? 'rgba(34,197,94,0.1)' : 'rgba(234,179,8,0.1)',
