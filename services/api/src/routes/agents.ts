...
agentRoutes.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const agentId = req.body.agent_id || db.collection(COLLECTION).doc().id
    const data = { ...req.body, agent_id: agentId, created_at: req.body.created_at || now, updated_at: now, _created_by: (req as any).user?.email || 'api' }
...
agentRoutes.patch('/:id', async (req: Request, res: Response) => {
  try {
    ...
    delete updates.agent_id; delete updates.id; delete updates.created_at
