...
import type { ProducerDTO, ProducerCreateDTO, ProducerUpdateDTO } from '@tomachina/core'
...
producerRoutes.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    res.json(successResponse<ProducerDTO>(stripInternalFields({ id: doc.id, ...doc.data() } as Record<string, unknown>) as unknown as ProducerDTO))
  } catch (err) {
...
producerRoutes.post('/', async (req: Request, res: Response) => {
  try {
    ...
    res.status(201).json(successResponse<ProducerCreateDTO>({ id: producerId, ...data } as unknown as ProducerCreateDTO))
  } catch (err) {
...
producerRoutes.patch('/:id', async (req: Request, res: Response) => {
  try {
    ...
    res.json(successResponse<ProducerUpdateDTO>(stripInternalFields({ id: updated.id, ...updated.data() } as Record<string, unknown>) as unknown as ProducerUpdateDTO))
  } catch (err) {
