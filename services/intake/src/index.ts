 ...
 */
export const spcIntake = onRequest(
 ...
 */
export const spcIntakeScheduled = onSchedule(
 ...
 * Trigger: Cloud Scheduler every 5 minutes.
 */
export const meetIntake = onRequest(
  { region: 'us-central1', timeoutSeconds: 120, memory: '256MiB' },
  async (_req, res) => {
...
export const meetIntakeScheduled = onSchedule(
  { schedule: 'every 5 minutes', region: 'us-central1', timeoutSeconds: 120, memory: '256MiB' },
  async () => {
    const result = await scanMeetRecordings()
 ...
 * Trigger: Cloud Scheduler every 5 minutes.
 */
export const mailIntake = onRequest(
  { region: 'us-central1', timeoutSeconds: 120, memory: '256MiB' },
  async (_req, res) => {
...
export const mailIntakeScheduled = onSchedule(
  { schedule: 'every 5 minutes', region: 'us-central1', timeoutSeconds: 120, memory: '256MiB' },
  async () => {
    const result = await scanMailIntake()
 ...
 * Trigger: Cloud Scheduler every 5 minutes.
 */
export const emailIntake = onRequest(
...
export const emailIntakeScheduled = onSchedule(
  { schedule: 'every 5 minutes', region: 'us-central1', timeoutSeconds: 180, memory: '512MiB' },
  async () => {
    const result = await scanEmailInboxes()
 ...
 * Trigger: Cloud Scheduler every 5 minutes.
 */
export const megazordDriveWatch = onRequest(
  { region: 'us-central1', timeoutSeconds: 180, memory: '512MiB' },
  async (_req, res) => {
    const result = await scanDriveIntake()
    console.log(`MEGAZORD Drive Watch: ${result.files_routed} routed, ${result.files_failed} failed`)
    res.json({ success: result.success, data: result })
  }
)

export const megazordDriveWatchScheduled = onSchedule(
  { schedule: 'every 5 minutes', region: 'us-central1', timeoutSeconds: 180, memory: '512MiB' },
  async () => {
    const result = await scanDriveIntake()
    console.log(`MEGAZORD Drive Watch (scheduled): ${result.files_routed} routed, ${result.errors.length} errors`)
  }
)
 ...
 */
export const queueStatus = onRequest(
