 ...
 */
async function findPartnerSlugByDomain(domain: string): Promise<string | null> {
  ...
  return (doc.data().slug as string) ?? null
}
 ...
 */
export const onPartnerUserCreate = beforeUserCreated(
  { region: 'us-central1' },
  async (event) => {
 ...
 */
export const refreshPartnerClaims = onRequest(
