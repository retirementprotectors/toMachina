 ...
 */
export async function scanMeetRecordings(): Promise<MeetScanResult> {
    ...
    for (const folder of foldersToScan) {
      try {
        ...
        for (const file of files) {
          ...
          if (!meta.is_audio && !meta.is_transcript) continue
          ...
          if (meta.is_audio) result.new_recordings++
          else result.new_transcripts++
        }
      } catch (err) {
