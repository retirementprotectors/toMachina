...
export interface DriveWatchResult {
  ...
  routing_summary: Array<{ filename: string; rangerId: string; status: string }>
  errors: string[]
}
 ...
 */
function routeFile(filename: string, mimeType: string): FileRouting {
 ...
 */
export async function scanDriveIntake(): Promise<DriveWatchResult> {
  const result: DriveWatchResult = {
    success: true,
    files_found: 0,
    files_routed: 0,
    files_failed: 0,
