...
import { scanInbox, markAsRead } from './lib/gmail-scanner.js';
...
function classifyPriority(msg) {
    ...
    if (subject.includes('urgent') || subject.includes('asap') || subject.includes('rush'))
        return 'high';
    if (subject.includes('fyi') || subject.includes('no action'))
        return 'low';
    return 'normal';
}
/**
            ...
            try {
                ...
                for (const msg of messages) {
                    ...
                    if (msg.attachments.length === 0) {
                        ...
                        // Still mark as read
                        try {
                            await markAsRead(config.address, msg.messageId);
                        }
                        catch { /* noop */ }
                        continue;
                    }
...
//# sourceMappingURL=email-intake.js.map