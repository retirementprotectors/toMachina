...
/* ─── Cloud Functions ─── */
export const onClientWrite = onDocumentWritten({ document: 'clients/{clientId}', region: 'us-central1' }, async (event) => {
    ...
    if (type === 'contact_updated' && fields_changed.length === 0)
        return;
...
export const onAccountWrite = onDocumentWritten({ document: 'clients/{clientId}/accounts/{accountId}', region: 'us-central1' }, async (event) => {
    ...
    const { fields_changed, summary } = diffFields(beforeData, afterData);
    if (type === 'account_updated' && fields_changed.length === 0)
        return;
    const col = notificationsRef();
    const ref = col.doc();
    await ref.set({
