 ...
 * Medicare accounts show enrollment/comparison/card.
 * Annuity accounts show policy/statement/illustration.
 * Found docs are clickable (preview), missing docs show gap.
 */
    ...
    return (
      <div className="flex gap-2 flex-wrap">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="h-9 w-32 rounded-lg bg-[var(--bg-surface)] animate-pulse" />
        ))}
      </div>
    )
  }
      ...
      <div className="flex flex-wrap gap-1.5">
        {found.map((doc) => (
          ...
          >
            ...
            {doc.count > 1 && (
              <span className="rounded-full bg-[color-mix(in_srgb,var(--portal)_15%,transparent)] px-1.5 text-[10px] text-[var(--portal)]">
                {doc.count}
              </span>
            )}
          </button>
